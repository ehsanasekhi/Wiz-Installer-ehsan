...
if(file_exists("panel/assets/js/Licence.DevAMiR")){ header("Location: /?action=installed"); }

# -------------------------------------- #

$TOKEN  = $_REQUEST['token']    ?? die('token is not set');
$UserID = $_REQUEST['userid']   ?? die('userid is not set');
$DBname = $_REQUEST['DBname']   ?? die('DBname is not set');
$DBuser = $_REQUEST['DBuser']   ?? die('DBuser is not set');
$DBpass = $_REQUEST['DBpass']   ?? die('DBpass is not set');
$URL    = $_REQUEST['url']      ?? die('url is not set');
...
if($connection->connect_error){
    ...
    $getKey = rand(11111, 99999);
    file_put_contents("Licence" . $getKey . ".DevAMiR", $getKey);
    file_put_contents("panel/assets/js/Licence.DevAMiR", $getKey);
...
$base   = file_get_contents("bot/baseInfo.php");
$base   = str_replace('^^TOKEN^^', $TOKEN, $base);
$base   = str_replace('^^admin^^', $UserID, $base);
$base   = str_replace('^^DBname^^', $DBname, $base);
$base   = str_replace('^^DBuser^^', $DBuser, $base);
$base   = str_replace('^^DBpassword^^', $DBpass, $base);
$base   = str_replace('^^url^^', $URL, $base);
file_put_contents("bot/baseInfo.php", $base);

# -------------------------------------- #

$webHook = file_get_contents("https://api.telegram.org/bot" . $TOKEN . "/setwebhook?url=" . $URL . "/bot/bot.php");
file_get_contents("https://api.telegram.org/bot" . $TOKEN . "/sendmessage?text=%E2%9D%95%20%D8%AA%D8%A8%D8%B1%DB%8C%DA%A9!%0D%0A%0D%0A%E2%9D%87%EF%B8%8F%20%D8%B1%D8%A8%D8%A7%D8%AA%20%D9%88%DB%8C%D8%B2%D9%88%DB%8C%D8%B2%20%D8%A8%D8%A7%20%DA%A9%D9%85%DA%A9%20%D8%A8%D8%B3%D8%AA%D9%87%20%D9%86%D8%B5%D8%A8%20%D8%A2%D8%B3%D8%A7%D9%86%20%C2%AB%D9%88%DB%8C%D8%B2%20%D8%A7%DB%8C%D9%86%D9%90%D8%B3%D8%AA%D8%A7%D9%84%D8%B1%C2%BB%20%D8%A8%D8%B1%D8%B1%D9%88%DB%8C%20%D9%87%D8%A7%D8%B3%D8%AA%20%D8%B4%D9%85%D8%A7%20%D9%86%D8%B5%D8%A8%20%D9%88%20%D9%81%D8%B9%D8%A7%D9%84%20%D8%B4%D8%AF.%0D%0A%0D%0A%D9%84%D8%B7%D9%81%D8%A7%20%D8%A8%D9%87%20%D9%BE%D8%B1%D9%88%DA%98%D9%87%20%E2%AD%90%EF%B8%8F%20%D8%A8%D8%AF%D9%87%DB%8C%D8%AF.&chat_id=" . $UserID);

if(json_decode($webHook, true)['ok'] == 1){
    header("Location: " . $URL . "/?action=ok");
}else{
    header("Location: " . $URL . "/?action=nok");
}
